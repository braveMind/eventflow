...
public class StreamHelper {
	...
	public static InputStream getStreamFromFile(File file) {
		...
		try {
			if (!file.exists()) {
				throw new EventFlowException("file " + file + " doesn't exist");
			}
			if (file.isDirectory()) {
				throw new EventFlowException("file " + file + " is a directory");
			}
			stream = new FileInputStream(file);

		} catch (Exception e) {
			throw new EventFlowException("couldn't access file " + file + ": "
					+ e.getMessage(), e);
		}
		return stream;
	}
	...
	public static InputStream getStreamFromClasspath(String resourceName) {
		...
		if (stream == null) {
			throw new EventFlowException("resource " + resourceName
					+ " does not exist");
		}
		return stream;
	}
	...
	public static InputStream getStreamFromUrl(URL url) {
		...
		try {
			...
			throw new EventFlowException("couldn't open URL stream", e);
		}
		return stream;
	}
